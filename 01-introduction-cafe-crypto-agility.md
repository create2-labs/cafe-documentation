...
For **Crypto Policy Management (CPM)**, wallet state is additionally described by a **versioned export contract** (`cafe.discovery.wallet.observed` v0.1): normalized account kind, algorithms, numeric chain IDs, public-key exposure, and a PQ posture summary. Discovery owns scan persistence and detail projection; CPM depends on the contract, not on Discovery database layouts. Details: [Developer Guide — Discovery to CPM](./03-cafe-developer-guide.md#discovery-to-cpm-observation-contract) and [Discovery README — Data structure](https://github.com/create2-labs/cafe-discovery/blob/main/README.md#data-structure-cpm-export-contract).
